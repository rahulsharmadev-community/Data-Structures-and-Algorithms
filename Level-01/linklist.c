...
void insertAt (Node *_node, int _index, int _element){
    
     auto void _insertAt(Node *node, int index, int element);
     _insertAt(_node, _index, _element);
     
    int i;
    void _insertAt(Node *node, int index, int element){
       if(index<0) {
    ...
    if (index == 0 && i == 0){
	 ...
	 i++;
    _insertAt(&((*node)->next), index, element);  
    }
