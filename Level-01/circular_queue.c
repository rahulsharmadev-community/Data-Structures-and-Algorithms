...
int main()
{
    ...
    do{ 
        ...
        if(input==1) enQueue(queue,&f,&r, getInput("Enter an element: "));
        else if(input==2) deQueue(&f,&r);
        display(queue,f,r);
    }while(input!=0);
    printf("Exit.");
    return 0;
...
void enQueue(int* queue, int* f, int* r, int element){
    if((*f==0 && *r==MAX-1)|| (*r+1)%MAX == *f) return;
...
void display(int* queue, int f, int r){
    ...
    if ((f==0 && r==MAX-1)|| (r+1)%MAX == f) printf("<----Queue is full");
    printf("\n");
}
